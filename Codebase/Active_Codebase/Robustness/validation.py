...
def validate_string(input_string, length=None, contain_substr=None):
    ...
    if (contain_substr is not None):
        try:
            contain_substr = validate_list(contain_substr)
            if not any(substr in input_string for substr in contain_substr):
                raise ValueError('Input string does not contain given '
                                 'substrings.'
                                 '    --Kyubey')
